package ro.ciacob.maidens.controller {
	...
	public class NativeAppsWrapper {
		...
		private static const FILE_COUNT_NUM_CHARS:int = 3;
		...
		private static const SVG_FILENAME_TEMPLATE:String = '%s%d.svg';
		...
		public function NativeAppsWrapper() {
			...
			if (_tempDirectory == null) {
				...
				var dirName:String = TEMP_DIR_NAME_TEMPLATE.replace('%s', Strings.
					generateUniqueId(_uidsPool, UID_LENGTH));
		...
		public function abcToMIDI(abcMarkup:String, settings:Object, callback:Function):void {
			...
			_callNativeApp($SCRIPT_MAKE_MIDI, completionMessage, [session, _tempDirectory.
				nativePath]);
		}
			...
			var complete_callback:Function = function(data:Object):void {
				...
				if (appFileName == $SCRIPT_MAKE_PDF) {
					if (arguments.indexOf(session) >= 0) {
						DEFAULT_PTT_PIPE.unsubscribe(NATIVE_HAS_COMPLETED, complete_callback);
						DEFAULT_PTT_PIPE.unsubscribe(NATIVE_HAS_STDERR, _onScriptError);
						var pdfFile:File = _tempDirectory.resolvePath(session.concat(CommonStrings.
							DOT, FileTypes.PDF));
						callback(pdfFile);
					}
				}
		...
		public function midiToWav(midi:File, settings:Object, callback:Function):void {
			...
			var complete_callback:Function = function(data:Object):void {
				...
				if (appFileName == $SCRIPT_MAKE_WAV) {
					if (arguments.indexOf(session) >= 0) {
						DEFAULT_PTT_PIPE.unsubscribe(NATIVE_HAS_COMPLETED, complete_callback);
						DEFAULT_PTT_PIPE.unsubscribe(NATIVE_HAS_STDERR, _onScriptError);
						var wavFile:File = _tempDirectory.resolvePath(session.concat(CommonStrings.
							DOT, FileTypes.WAV));
						callback(wavFile);
					}
				}
			...
			DEFAULT_PTT_PIPE.subscribe(NATIVE_HAS_COMPLETED, complete_callback);
			DEFAULT_PTT_PIPE.subscribe(NATIVE_HAS_STDERR, _onScriptError);
			_callNativeApp($SCRIPT_MAKE_WAV, completionMessage, [session, _tempDirectory.
				nativePath]);
		}
		...
		private function _extractSVGFiles(sessionName:String):Array {
			var ret:Array = [];
			var count:int = 1;
			do {
				var countToken:String = Strings.padLeft(count.toString(), CommonStrings.
					ZERO, FILE_COUNT_NUM_CHARS);
				var fileName:String = SVG_FILENAME_TEMPLATE.replace('%s', sessionName).
					replace('%d', countToken);
				var file:File = _tempDirectory.resolvePath(fileName);
				if (file.exists) {
					ret.push(file);
					count++;
				} else {
		...
		private function _haltExecution(template:String, detail:String):void {
			var msg:String = template.replace('%s', detail);
			throw(new Error(msg));
		}

		private function _haveText(text:String, ... inStrings):Boolean {
		 ...
		 */
		private function _makeSessionFile(fileContent:ByteArray, fileExtension:String,
		...
		private function _onScriptError(data:Object):void {
			DEFAULT_PTT_PIPE.unsubscribe(NATIVE_HAS_STDERR, _onScriptError);
		...
		private function _runBatchScript(scriptName:String, successSTDOUT:String,
			...
			var templateFile:File = FileAssets.TEMPLATES_DIR.resolvePath(FileAssets.
				LAUNCHER_QUERY_TEMPLATE);
			...
			launcherDataSrc[DataKeys.CONTENT][LauncherKeys.ACTION] = LauncherKeys.
				COMMAND;
			launcherDataSrc[DataKeys.CONTENT][LauncherKeys.PATH] = _nativeAppsFolder.
				resolvePath(scriptName).nativePath;
		...
		private function _runNativeApp(appName:String, successSTDOUT:String, args:Array =
			...
			launcherDataSrc[DataKeys.CONTENT][LauncherKeys.ACTION] = LauncherKeys.
				APPLICATION;
			launcherDataSrc[DataKeys.CONTENT][LauncherKeys.PATH] = _nativeAppsFolder.
				resolvePath(appName).nativePath;
			launcherDataSrc[DataKeys.CONTENT][LauncherKeys.DETAILS] = launcherQuery;
			var launcherData:ProjectData = new ProjectData;
			var importer:PlainObjectImporter = new PlainObjectImporter;
			importer.importData(launcherDataSrc, launcherData);
