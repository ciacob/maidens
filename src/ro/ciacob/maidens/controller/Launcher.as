package ro.ciacob.maidens.controller {
	...
	public class Launcher implements ILauncher {
		...
		private static const SEPARATOR:String = '/** %s */';
		...
		private var _currItemDescription:String;
		private var _details:String;
		private var _dispatcher:IEventDispatcher;
		private var _errorDetail:String;
		private var _file:File;
		private var _flatItemsList:Array;
		...
		public function attemptToExit():void {
		...
		public function launch(srcData:ProjectData):void {
			...
			_flatItemsList = _flattenData(_srcData);
			...
			if (_srcData != null) {
				...
				var haveDetails:Boolean = (_details != null && !Strings.isEmpty(Strings.
					trim(_details)));
				if (haveDetails) {
				...
				if (_actionType == LauncherKeys.WEB || _actionType == LauncherKeys.
					FILE || _actionType == LauncherKeys.FOLDER || _actionType ==
					LauncherKeys.APPLICATION) {

					var havePath:Boolean = (_path != null && !Strings.isEmpty(Strings.
						trim(_path)));
					if (!havePath) {
					...
					if (_actionType != LauncherKeys.WEB) {
						...
						if (_actionType == LauncherKeys.FILE || _actionType == LauncherKeys.
							FOLDER) {
		...
		private function _executeScript(interpreter:File, workingDirectory:File,
			...
			for (var header:String in KnownScripts.TABLE) {
				var interpreterName:String = KnownScripts.TABLE[header][KnownScripts.
					INTERPRETER];
		...
		private function _findActionType(path:String, details:String):String {
		...
		private function _getStartupArguments(rawText:String):Vector.<String> {
		...
		private function _getWorkingDir(rawText:String):File {
		...
		private function _onNPErrorData(event:ProgressEvent):void {
			...
			var processError:String = process.standardError.readUTFBytes(process.
				standardError.bytesAvailable);
		...
		private function _runApplication(file:File, startupArguments:Vector.<String>,
			...
			if (NativeProcess.isSupported) {
				...
				var haveStartupArguments:Boolean = (startupArguments != null && startupArguments.
					length > 0);
		...
		private function _runCommand():void {
			...
			if (haveDetails) {
				for (header in KnownScripts.TABLE) {
					if (_details.indexOf(header) == 0) {
						interpreterFileName = KnownScripts.TABLE[header][KnownScripts.
							INTERPRETER];
						break;
					}
				}
			...
			if (interpreterFileName == null) {
				...
				if (havePath) {
					for (header in KnownScripts.TABLE) {
						var fileType:String = KnownScripts.TABLE[header][KnownScripts.
							FILE_TYPE];
				...
				// Or maybe it is a path relative to the system folder
					...
					if (_scriptingHomePath != null) {
						...
						if (scriptingHome.exists && scriptingHome.isDirectory) {
							resolvedInterpreterFileName = scriptingHome.resolvePath(interpreterFileName).
								nativePath;
				...
				if (haveDetails) {
					...
					for (header in KnownScripts.TABLE) {
						var interpreterName:String = KnownScripts.TABLE[header][KnownScripts.
							INTERPRETER];
						if (interpreterName == interpreterFileName) {
							scriptFileType = KnownScripts.TABLE[header][KnownScripts.
								FILE_TYPE];
							break;
						}
					}
					scriptFile = File.applicationStorageDirectory.resolvePath(SCRIPTS_HOME).
						resolvePath(scriptFileName.concat('.', scriptFileType));
			...
			// Any meaningfull status, from now on, is to be returned by the 
			// `executeScript` method.
			_executeScript(interpreter, workingDirectory, scriptFile, startupArguments);
			return;
		}

		private function _updateProgressStats():void {
			var currItem:ProjectData = _flatItemsList.shift();
			_currItemDescription = currItem.getContent(LauncherKeys.LABEL);
		}
	}
}
