package ro.ciacob.maidens.model {
	...
	public class GeneratorsWiringMap {
		...
		private const INTRO : String = 'Class `GeneratorsWiringMap` ';
		private const ADD : String = 'cannot `add()`: ';
		private const GET : String = 'cannot `get()`: ';
		private const HAS : String = 'cannot `has()`: ';
		private const DEL : String = 'cannot `remove()`: ';
		private const IMPLICIT_OVERWRITE : String = 'ID already exists. Use the `replace` argument, or verify with `has()`.';
		private const MISSING_ID : String = 'ID does not exist. Use `has()` to verify.';
		private const NULL_ID : String = 'argument `generatorID` cannot be null.';
		private const NULL_GENERATOR : String = 'argument `generator` cannot be null.';
		...
		private function _byConnectionNumber (generatorIdA : GeneratorInstance, generatorIdB : GeneratorInstance) : int {
