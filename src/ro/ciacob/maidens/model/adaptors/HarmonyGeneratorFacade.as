...
//					annotation = annotationRows.join('`') ;
     ...
     * produce output.
     */
    private function _buildRequest(targets:Array):IMusicRequest {
            ...
            if (parameter.type == CoreOperationKeys.TYPE_ARRAY) {
                ...
                switch (parameter.type) {
                    case CoreOperationKeys.TYPE_INT:
                        ...
                        // (for the time being, we only have arrays and integers).
                }
            }
        }
        _parameters.forEach(mover);
        ...
        var _onSectionsListReady:Function = function (apiName:String, sections:Vector.<ProjectData>):void {
            ...
            var timeSignatureEntry:ITimeSignatureEntry = null;
            var k:int;
            ...
            for (k = 0; k < sections.length; k++) {
                ...
                for (i2 = 0; i2 < measuresInSection.length; i2++) {
                    ...
                    // Update existing time signature span if this is a subsequent occurrence of
     ...
     * NOTE:
     * The algorithm is:
     * - determine the number of eligible staves: this is the smallest of the number of autonomous voices of the
     *   instrument or the instrument's *current* number of staves ("current", because the user can alter this);
     * - loop through the instrument's eligible staves in reverse order, and look for a pitch allocation that
     ...
     */
    private static function _evenOutVoices(sortedAllocations:Vector.<IPitchAllocation>):void {
        var currInstrumentUid:String = null;
        var currInstrumentNumStaves:int;
        var voiceSearchIndex:int = int.MAX_VALUE;
        var currAllocation:IPitchAllocation;
        var tmpInstrument:IMusicalInstrument = null;
     ...
     */
    private function _onAbortRequest(...ignore):void {
        $pipe.unsubscribe(GeneratorKeys.GEN_ABORT_REQUESTED, _onAbortRequest);
        _generator.abort();
    }
}
